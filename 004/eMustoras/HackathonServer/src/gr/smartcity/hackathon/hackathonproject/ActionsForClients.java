...
public class ActionsForClients extends Thread {
    ...
    Vec2<String, ArrayList<String>> locations;
    ...
    public void run() {
        ...
        try {
            ...
            if(initial.getMode() == 'P' || initial.getMode() == 'C' || initial.getMode() == 'M') geoLocationService();
            if(initial.getMode() == 'R' || initial.getMode() == 'I') ratingSupportService();
        } catch (ClassNotFoundException | IOException e) {
            e.printStackTrace();
        }
    }
    
    public void geoLocationService() {
        try {
            if(initial.getMode() == 'P') {                                        // Professional Greeting
                ...
                synchronized(HackathonServer.users) {
                    HackathonServer.users.put(uid, coord);
                }
                System.out.println("Users stored: " + HackathonServer.users.size());
            } else if (initial.getMode() == 'M') {                            // Google Maps API
                System.out.println("Initializing Google Maps.");
                HashMap<String, Vec2<Float, Float>> response = new HashMap<>();
                locations = (Vec2<String, ArrayList<String>>) initial;
                for(String wid: locations.getYValue()) {
                    if(HackathonServer.users.containsKey(wid)) {
                        Vec2<Float, Float> loc = HackathonServer.users.get(wid);
                        response.put(wid, loc);
                    }
                }
    ...
    public void ratingSupportService() {
        try {
            if (initial.getMode() == 'R') {                            // Rating Assignment
